...
uses
 Windows, Messages, SysUtils, Classes, Graphics, WinSock, proto, pworks, zlib;
...
type
 ...
 public
  ...
  procedure AurgorizeMe(EMail, Text: string);
...
procedure Register;
begin
 RegisterComponents('Samples', [TMailClient]);
end;
...
begin
 Result:= -1;
 for i:= 0 to Length(FItems) - 1 do
  if CompareStr(FItems[i].EMail, EMail) = 0 then
  begin
   Result:= i;
   Break;
  end;
       ...
       begin
        ...
        while ID < GroupCount do
         ...
         begin
          ...
          if Assigned(FOnContactGroup) then
        ...
        while Offset < Pack.dlen do
         ...
         begin
          ...
          if Assigned(FOnContact) then
           FOnContact(Self, GroupID, EMail, Nick,
            DStatusTo(Status), DInvisStateTo(Flag));
         end;
...
procedure TMailClient.AurgorizeMe;
var
 Pack: TMRIMPacket;
 Data: Pointer;
begin
